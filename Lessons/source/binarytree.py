...
class BinarySearchTree(object):
    ...
    def insert(self, item):
        ...
        # print("self", self, self.root, "item", item)
        ...
        elif item > parent.data: # maybe else?
    ...
    def _find_successcor(self, node):
    ...
    def delete(self, item):
        ...
        if node.is_leaf(): # node has no children
            if node == self.root: 
                self.root = None # when node is root, then delete root
                return
        
            # compare parent data to item
            if item < parent.data: 
                parent.left = None
                return
            else: 
        ...
        if node.left and node.right: # node has both children 
            pred = self._find_predecessor(node) # find predecessor
            succ = self._find_successcor(node) # find successor

            if succ: 
                print("successor", succ.data)
                succ_node = self._find_node_iterative(succ.data) # get successor node
                # replace node's data with successor's data
                node.data = succ.data 
                # delete successor node (not working)
                succ_node = None
                print(succ_node, succ, self.root)
                return
            else: 
                node.data = pred.data
                pred = None
        ...
        if item < parent.data: # all the one child cases
    ...
    def _traverse_in_order_recursive(self, node, visit):
        ...
        the number of edges
        Memory usage: O(l) where l is the maximum depth of the BST
        O(logn) if balanced 
        approaches O(n) if not balanced  
        """
        if node: 
